...
详细模式下，每个项目默认预览三条活跃任务（未完成及待验收），其余点击展开；不需要验收的已完成任务默认折叠。简洁模式保留项目分组，每条任务只显示单行标题和状态，默认显示全部活跃任务；已验收仍折叠。内容超出时在窗口内滚动，任务更新不会强制展开项目。
...
- **系统通知**：任务变为受阻、需要你补充或等你验收时，弹出 Windows 通知（浮窗在托盘运行即可）。通知会一直留在屏幕上直到你处理，点通知或「打开看板」显示浮窗，点「稍后」关闭；一次出现多条时合并为一条。可在设置中关闭。
- **自动归档**：已验收的任务在若干天无变化后自动归档，默认 7 天，可选 1、3、7、30 天或关闭；等待验收和旧版未经过人工验收的任务不会被自动归档。
